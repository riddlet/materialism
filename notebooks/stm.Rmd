...
#look at those most associated with...
#dat %>% arrange(desc(low)) %>% head(., 20)
#dat %>% arrange(desc(med)) %>% head(., 20)
#dat %>% arrange(desc(high)) %>% head(., 20)
...
It is the latter part of the output that we are most interested in. This part of the topic model has something like a regression built into it (technically a variant of a lasso regression). The outcome of this regression is the probability that word $k$ is pulled from a distribution, and the predictors are the baseline word distribution (i.e. the intercept), along with deflections for each level of the covariate (note: it is not possible to specify continuous covariates for this part of the model). We can optionally specify topic-covariate interactions (giving us Low, Medium, and High words for each topic), however, based on my understanding of the research question, this is not of particular interest. By not specifying interactions, we are also better able to make inferences for those quantites that we care about, as our data will be concentrated in a lower dimensional space.
...
At this point, it's difficult to say anything concretely about some kind of broad pattern, but I certainly have some ideas.